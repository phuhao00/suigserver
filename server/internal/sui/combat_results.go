...
func NewCombatResultsSuiService(client *SuiClient, packageID, moduleName, senderAddress, gasObjectID string) *CombatResultsSuiService {
...
func (s *CombatResultsSuiService) RecordCombatOutcome(data CombatResultData, gasBudget uint64) (models.TransactionBlockResponse, error) {
	...
	log.Printf("Preparing to record combat outcome for log %s: Winner %s, Loser %s. Package: %s, Module: %s, Function: %s",
	...
	log.Printf("Transaction prepared for RecordCombatOutcome (log %s). TxBytes: %s",
...
func (s *CombatResultsSuiService) GetCombatOutcome(combatOutcomeObjectID string) (models.SuiObjectResponse, error) {
	log.Printf("Fetching combat outcome for object ID %s.\n", combatOutcomeObjectID)
	...
	log.Printf("Successfully fetched combat outcome object %s.", combatOutcomeObjectID)
